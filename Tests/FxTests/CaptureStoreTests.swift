...
final class CaptureStoreTests: XCTestCase {
    @MainActor
    func testMovedManagedLibraryRepairsItsPersistedAbsolutePath() throws {
        let fileManager = FileManager.default
        let base = fileManager.temporaryDirectory
            .appendingPathComponent("FxMovedLibrary-\(UUID().uuidString)", isDirectory: true)
        let oldRoot = base.appendingPathComponent("OldAppName", isDirectory: true)
        let newRoot = base.appendingPathComponent("Fx", isDirectory: true)
        defer { try? fileManager.removeItem(at: base) }

        let original = CaptureStore(rootURL: oldRoot)
        let saved = try original.saveText("Migrated context")
        try fileManager.moveItem(at: oldRoot, to: newRoot)

        let reloaded = CaptureStore(rootURL: newRoot)
        let library = try XCTUnwrap(reloaded.selectedCollection)
        XCTAssertEqual(reloaded.folderURL(for: library), newRoot.appendingPathComponent("Library"))
        XCTAssertEqual(reloaded.records.first(where: { $0.id == saved.id })?.text, "Migrated context")

        let configuration = try String(
            contentsOf: newRoot.appendingPathComponent("collections.json"),
            encoding: .utf8
        )
        XCTAssertTrue(configuration.contains(newRoot.appendingPathComponent("Library").path))
        XCTAssertFalse(configuration.contains(oldRoot.path))
    }
    ...
    func testLibraryWorkingDirectoryPersistsWithoutMovingLibraryStorage() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("FxWorkingDirectory-\(UUID().uuidString)", isDirectory: true)
    ...
    func testEveryManagedItemTypePersistsOnARecord() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("FxItemTypes-\(UUID().uuidString)", isDirectory: true)
    ...
    func testSubagentDefinitionAndEditableBodyPersist() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("FxSubagentItem-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testSodaSystemPromptInstallsOnceIntoLibraryEvenWhenAnotherCollectionIsSelected() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("FxSodaSystem-\(UUID().uuidString)", isDirectory: true)
    ...
    func testJustOneAPIInstallsOneSchemaContextPerPlatformExactlyOnce() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("FxJustOneAPIContexts-\(UUID().uuidString)", isDirectory: true)
        defer { try? fileManager.removeItem(at: root) }
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
    ...
    func testDiscardingAnUncommittedSessionDraftRemovesItsFolderPermanently() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("FxDiscardSession-\(UUID().uuidString)", isDirectory: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testCollectionStoresEachCaptureInItsOwnFolderAndReloadsIt() throws {
        ...
        let testRoot = fileManager.temporaryDirectory
            .appendingPathComponent("FxTests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testEveryContentTypeCanReceiveAnIndependentCover() throws {
        let fileManager = FileManager.default
        let testRoot = fileManager.temporaryDirectory
            .appendingPathComponent("FxCoverTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? fileManager.removeItem(at: testRoot) }
    ...
    func testImportedImageCoverCreatesPersistentGalleryPreview() async throws {
        let fileManager = FileManager.default
        let testRoot = fileManager.temporaryDirectory
            .appendingPathComponent("FxCoverPreviewTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? fileManager.removeItem(at: testRoot) }
    ...
    func testDefaultDetailsAndFileTabsPersistAsRealFiles() async throws {
        let fileManager = FileManager.default
        let testRoot = fileManager.temporaryDirectory
            .appendingPathComponent("FxDetailTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? fileManager.removeItem(at: testRoot) }
    ...
    func testContextCombinesMetadataOriginalContentAndTypedTabs() async throws {
        let fileManager = FileManager.default
        let testRoot = fileManager.temporaryDirectory
            .appendingPathComponent("FxContextTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? fileManager.removeItem(at: testRoot) }
    ...
    func testRenamingATabRenamesTheBackingFolder() throws {
        let fileManager = FileManager.default
        let testRoot = fileManager.temporaryDirectory
            .appendingPathComponent("FxRenameTests-\(UUID().uuidString)", isDirectory: true)
        let appData = testRoot.appendingPathComponent("AppData", isDirectory: true)
