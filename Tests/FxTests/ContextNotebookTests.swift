...
final class ContextNotebookTests: XCTestCase {
    ...
    func testFxModelSettingsLoadsStringsObjectsAndDefaultModel() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("FxModelSettings-\(UUID().uuidString).json")
    ...
    func testReasoningEffortPersistsAndExportsToRuntime() throws {
        ...
        let encoded = try JSONEncoder.fx.encode(notebook)
        let decoded = try JSONDecoder.fx.decode(ContextNotebook.self, from: encoded)
    ...
    func testVersionOnePresetQueryTitleMigratesToEmpty() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("FxQueryMigrationTests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testVersionTwoQueryTitleBecomesContentWhenBodyIsEmpty() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("FxSingleQueryFieldTests-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testVersionThreeDefaultSystemTitleMigratesWithoutRenamingCustomPrompts() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("FxSystemTitleMigrationTests-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testNotebookPersistsAndExportsFxAgentSchema() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("FxNotebookTests-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testReferencedImageContentExportsAsAnImageAttachment() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("FxCoverContextTests-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testQueryImagePersistsAndExportsAsMultimodalContent() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("FxQueryImageTests-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testMarkdownParserRemovesColumnsThatAreEntirelyEmpty() {
        let markdown = """
        | 类目产品分类平台类目路径 | | | | |
        | --- | --- | --- | --- | --- |
        | 床品套件类 | 四件套、六件套 | 床上用品 > 床品套件 | | |
        | 被芯类 | 羽绒被、蚕丝被 | 床上用品 > 被芯 | | |
        """

        XCTAssertEqual(ContextMarkdownParser.parse(markdown), [
            .table(
                headers: ["类目产品分类平台类目路径", "", ""],
    ...
    func testToolPresentationUsesSpecializedMinimalSummaries() {
        let bash = ContextToolPresentation.make(
            name: "bash",
            payload: .object([
                "intent": .string("Run the Swift test suite."),
        ...
        XCTAssertEqual(bash.summary, "Run the Swift test suite.")
        XCTAssertEqual(bash.command, "swift test")
        XCTAssertEqual(bash.metadata, "timeout 30s")

        let legacyBash = ContextToolPresentation.make(
            name: "bash",
            payload: .object(["command": .string("pwd")]),
            fallbackText: "",
            isResult: false
        )
        XCTAssertEqual(legacyBash.summary, "Run shell command")
        XCTAssertEqual(legacyBash.command, "pwd")
    ...
    func testCompletedRunIsSavedAsReloadableSessionItem() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("FxSessionTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        let persistedMessages = try JSONDecoder.fx.decode(
            [JSONValue].self,
    ...
    func testSavedSessionReloadsItsEditableNotebook() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("FxEditableSession-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testNotebookExternalizesHeavyEventsAndContinuationMessages() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("FxCompactSession-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        let persistedEvents = try JSONDecoder.fx.decode(
    ...
    func testSavingFreshEditorsCreatesIndependentSessionItems() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("FxIndependentSessions-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testUpsertingAnActiveSessionUpdatesTheSameRecordAndTranscript() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("FxSessionUpsertTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        XCTAssertEqual(persistedMessages, messages)
        let persistedNotebook = try JSONDecoder.fx.decode(
    ...
    func testSessionArtifactsWorkspaceIsCreatedInsideItsSessionFolder() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("FxSessionArtifactsWorkspace-(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
