...
test("registers a configurable OpenAI Responses model", async () => {
  const root = await mkdtemp(join(tmpdir(), "fx-model-registry-"));
...
test("registers Photonmark Codex models through the Responses endpoint", async () => {
  const root = await mkdtemp(join(tmpdir(), "fx-model-registry-"));
  const agentHome = join(root, "agent");
  const settingsPath = join(root, "settings.json");
  await mkdir(agentHome, { recursive: true });
  ...
  const registry = await loadModelRegistry(agentHome, settingsPath);
  const model = resolveModel(registry.models, "photonmark/gpt-6-astra");

  assert.equal(registry.defaultModel, "wuai/gemini-3.8-flash");
  assert.equal(model?.api, "openai-responses");
  assert.equal(model?.baseUrl, "https://codex.photonmark.com/openai/v1");
...
test("registers Gemini 3.8 Flash through the verified wuai OpenAI endpoint", async () => {
  const root = await mkdtemp(join(tmpdir(), "fx-model-registry-"));
  const agentHome = join(root, "agent");
  const settingsPath = join(root, "settings.json");
  await mkdir(agentHome, { recursive: true });
  ...
  const registry = await loadModelRegistry(agentHome, settingsPath);
  const model = resolveModel(registry.models, "wuai/gemini-3.8-flash");
