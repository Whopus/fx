...
test("completed runs release their cancellation listener", async () => {
  const faux = fauxProvider({ provider: "faux-lifetime" });
  ...
  }).run(base(), { signal: controller.signal });
  assert.equal(output.final, "finished");
  assert.equal(getEventListeners(controller.signal, "abort").length, 0);
});

test("cancellation before a prompt prevents provider work and releases listeners", async () => {
  for (const cancelDuringRoundStart of [false, true]) {
    const faux = fauxProvider({ provider: "faux-cancel-before-prompt" });
    let requested = false;
    const controller = new AbortController();
    if (!cancelDuringRoundStart) controller.abort();
    const output = await new PiRuntime({
      model: faux.getModel(),
      streamFn: () => { requested = true; throw new Error("unexpected provider call"); },
    }).run(base(), {
      signal: controller.signal,
      onEvent: async (event) => {
        if (cancelDuringRoundStart && event.type === "fx/round_start") controller.abort();
      },
    });
