...
function normalizeURL(value: unknown): string | undefined {
  ...
  try {
    ...
    // search_item_list_v1 currently returns g.search[0-9]*.alicdn.com.
    // Alibaba's certificate only covers one label below alicdn.com, so native
    // URLSession clients reject those otherwise valid image paths. The same
    // immutable image is served by Alibaba's canonical image CDN.
    if (/^g\.search\d*\.alicdn\.com$/i.test(url.hostname)) {
      url.hostname = "img.alicdn.com";
    }
