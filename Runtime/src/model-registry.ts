...
async function providerCredential(definition: PiProviderDefinition): Promise<{ apiKey: string; source: string } | undefined> {
  ...
  if (definition.keychainService) {
    const account = definition.keychainAccount?.trim() || "fx";
...
/**
 * Fx owns this registry. It preserves pi's provider/model semantics and
...
export async function loadModelRegistry(
  ...
  const reposRoot = process.env.FX_REPOS_ROOT ?? resolve(homedir(), "repos");
  const deepseekEnvPath = process.env.FX_DEEPSEEK_ENV ?? resolve(reposRoot, "soda/benchmarks/.env");
  ...
  const bailianEnvPath = process.env.FX_BAILIAN_ENV ?? resolve(reposRoot, "soda/benchmarks/.env.planora");
  ...
  const piDefault = settings?.defaultProvider && settings.defaultModel
    ...
    : undefined;
  const requestedDefault = process.env.FX_MODEL
