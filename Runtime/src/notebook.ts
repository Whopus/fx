...
import type { FxNotebook, FxRuntime, OutputCell, RunHooks } from "./types.ts";

export async function loadNotebook(path: string): Promise<FxNotebook> {
  const value: unknown = normalizeNotebook(JSON.parse(await readFile(path, "utf8")));
  validateNotebook(value);
  return value;
}

export function newNotebook(title = "Untitled Fx Agent"): FxNotebook {
  return {
    version: 1,
    id: randomUUID(),
    title,
...
export async function saveNotebook(path: string, notebook: FxNotebook): Promise<void> {
