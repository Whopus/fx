...
final class CaptureStore: ObservableObject {
    @Published private(set) var records: [CaptureRecord] = [] {
        didSet { gallerySnapshots.removeAll(keepingCapacity: true) }
    }
    ...
    init(rootURL overrideRootURL: URL? = nil) {
        ...
        rootURL = overrideRootURL
            ?? applicationSupport.appendingPathComponent("Fx", isDirectory: true)
    ...
    func gallerySnapshot(in space: CaptureSpace) -> GallerySnapshot {
        if let snapshot = gallerySnapshots[space] { return snapshot }
        var items: [GalleryItem] = []
        var hasher = Hasher()
        for record in records where !record.isTrashed && (record.space ?? .context) == space {
            let media = galleryMediaURLs(for: record)
            let item = GalleryItem(capture: record, coverPreviewURL: media.preview, coverVideoURL: media.video)
            items.append(item)
            hasher.combine(item.id)
            hasher.combine(item.aspectRatio)
        }
        let snapshot = GallerySnapshot(items: items, layoutSignature: hasher.finalize())
        gallerySnapshots[space] = snapshot
        return snapshot
    }
    ...
    func installSodaSystemPromptIfNeeded(sourceURL: URL) throws -> CaptureRecord? {
        ...
        let existing = readMetadata(in: libraryURL).first(where: { ($0.tags ?? []).contains(markerTag) })
        if existing != nil {
    ...
    func replaceCover(for id: UUID, from sourceURL: URL) async throws {
        ...
        defer {
            if !completed {
                ...
                if let previewURL { try? fileManager.removeItem(at: previewURL) }
            }
        }
        ...
        let previousRecord = try latestRecord(id: id, at: itemURL)
        ...
        try commit(updatedRecord, at: itemURL)
        completed = true
        removePreviousCoverFiles(for: previousRecord, in: itemURL, keeping: Set([fileName, previewName].compactMap { $0 }))
    }
    ...
    private func replaceCoverData(_ data: Data, for id: UUID) throws {
        ...
        var completed = false
        defer {
            if !completed { try? fileManager.removeItem(at: destinationURL) }
        }
        ...
        completed = true
        removePreviousCoverFiles(for: previousRecord, in: itemURL, keeping: Set([fileName]))
    }
    ...
    func replaceCoverVideo(for id: UUID, from sourceURL: URL) async throws {
        ...
        try commit(updatedRecord, at: itemURL)
        completed = true
        removePreviousCoverFiles(for: previousRecord, in: itemURL, keeping: Set([fileName, posterName]))
    }
    ...
    func removeCover(for id: UUID) throws {
        ...
        try writeMetadata(updatedRecord, to: itemURL)
        records[index] = updatedRecord
        removePreviousCoverFiles(for: previousRecord, in: itemURL, keeping: [])
    }
    ...
    func importDetailTabFile(for id: UUID, from sourceURL: URL) async throws -> CaptureDetailTab {
        ...
        do {
            var updatedRecord = try latestRecord(id: id, at: itemURL)
            var tabs = updatedRecord.detailTabs ?? []
            tabs.append(tab)
            updatedRecord.detailTabs = tabs
            try commit(updatedRecord, at: itemURL)
    ...
    func textSource(for record: CaptureRecord) -> CaptureTextSource {
        CaptureTextSource(
            record: record,
            originalURL: fileURL(for: record),
            tabURLs: Dictionary((record.detailTabs ?? []).compactMap { tab in
                fileURL(for: tab, in: record).map { (tab.id, $0) }
            }, uniquingKeysWith: { _, latest in latest })
        )
    }

    func originalTextContent(for record: CaptureRecord) -> String? {
        CaptureTextSource(record: record, originalURL: fileURL(for: record), tabURLs: [:]).originalText
    }

    func context(for record: CaptureRecord) -> String {
        textSource(for: record).context()
    }
    ...
    func prepareRuntime(
        ...
        let referencedIDs = Set(contextItems.compactMap(\.sourceRecordID))
        let fallbackIDs = Set(contextItems.lazy.filter {
            $0.body.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
        }.compactMap(\.sourceRecordID))
        var textSources: [CaptureTextSource] = []
        var mediaURLs: [UUID: URL] = [:]
        for record in records where !record.isTrashed && referencedIDs.contains(record.id) {
            if fallbackIDs.contains(record.id) { textSources.append(textSource(for: record)) }
            mediaURLs[record.id] = contextMediaURL(for: record)
        }
        let continuationURL = records.first {
            $0.id == sessionID && $0.space == .session && !$0.isTrashed
    ...
    func notebook(forSession sessionID: UUID) throws -> ContextNotebook {
        ...
        guard let data = try? Data(contentsOf: notebookURL),
              var notebook = try? JSONDecoder.fx.decode(ContextNotebook.self, from: data) else {
            throw CaptureStoreError.sessionNotebookUnavailable
        }
        ...
        if data.count > 1_000_000,
           let compactData = try? JSONEncoder.fx.encode(compactedNotebook(notebook)) {
    ...
    private func writeNotebookSession(
        ...
        try JSONEncoder.fx.encode(compactedNotebook(notebook)).write(
    ...
    private func decodeArtifact<Value: Decodable>(
        ...
        return try? JSONDecoder.fx.decode(type, from: data)
    }
    ...
    private func loadConfigurationOrBootstrap() {
        if let data = try? Data(contentsOf: configurationURL),
           var configuration = try? JSONDecoder.fx.decode(CollectionConfiguration.self, from: data) {
            let repairedMovedLibrary = repairMovedManagedLibrary(in: &configuration)
    ...
    /// Collection paths are persisted as absolute URLs. When the app data root is
    /// renamed (for example Curatez -> Fx), repair the built-in Library path while
    /// leaving user-added external collections untouched.
    private func repairMovedManagedLibrary(in configuration: inout CollectionConfiguration) -> Bool {
        let currentLibraryURL = rootURL
            .appendingPathComponent("Library", isDirectory: true)
            .standardizedFileURL
        guard fileManager.fileExists(atPath: currentLibraryURL.path) else { return false }

        var repaired = false
        for index in configuration.folders.indices {
            guard configuration.folders[index].name.localizedCaseInsensitiveCompare("Library") == .orderedSame else {
                continue
            }

            let storedLibraryURL = URL(
                fileURLWithPath: configuration.folders[index].path,
                isDirectory: true
            ).standardizedFileURL
            guard !fileManager.fileExists(atPath: storedLibraryURL.path),
                  storedLibraryURL != currentLibraryURL else { continue }

            configuration.folders[index].path = currentLibraryURL.path
            if let workingPath = configuration.folders[index].workingDirectoryPath {
                let storedPrefix = storedLibraryURL.path + "/"
                if workingPath == storedLibraryURL.path {
                    configuration.folders[index].workingDirectoryPath = currentLibraryURL.path
                } else if workingPath.hasPrefix(storedPrefix) {
                    let relativePath = String(workingPath.dropFirst(storedPrefix.count))
                    let candidate = currentLibraryURL.appendingPathComponent(relativePath).standardizedFileURL
                    if fileManager.fileExists(atPath: candidate.path) {
                        configuration.folders[index].workingDirectoryPath = candidate.path
                    }
                }
            }
            repaired = true
    ...
    private func migrateLegacyRecords(into libraryURL: URL) {
        guard let data = try? Data(contentsOf: legacyRecordsURL),
              let legacyRecords = try? JSONDecoder.fx.decode([CaptureRecord].self, from: data) else { return }
    ...
    private func loadRecords() {
        ...
        records = readMetadata(in: folder).sorted { $0.createdAt > $1.createdAt }
    }
    ...
    private func insert(_ record: CaptureRecord, in itemURL: URL) throws {
        try writeMetadata(record, to: itemURL)
        if selectedCollection.map({ folderURL(for: $0) }) == itemURL.deletingLastPathComponent() {
            records.insert(record, at: 0)
        }
    }

    /// Resolve again after suspension: insertion, deletion, or a collection
    /// switch may have invalidated every array index captured before an await.
    private func latestRecord(id: UUID, at itemURL: URL) throws -> CaptureRecord {
        if let record = records.first(where: { $0.id == id }), itemFolderURL(for: record) == itemURL {
            return record
        }
        guard let data = try? Data(contentsOf: itemURL.appendingPathComponent("metadata.json")),
              var record = try? JSONDecoder.fx.decode(CaptureRecord.self, from: data),
              record.id == id else {
            throw CaptureStoreError.collectionFolderUnavailable
        }
        record.containerFolderName = itemURL.lastPathComponent
        return record
    }

    private func commit(_ record: CaptureRecord, at itemURL: URL) throws {
        try writeMetadata(record, to: itemURL)
        if let index = records.firstIndex(where: { $0.id == record.id }),
           itemFolderURL(for: records[index]) == itemURL {
            records[index] = record
        }
    }

    private func writeMetadata(_ record: CaptureRecord, to itemURL: URL) throws {
        let data = try JSONEncoder.fx.encode(record)
        try data.write(to: itemURL.appendingPathComponent("metadata.json"), options: .atomic)
        startupMetadata?[itemURL.deletingLastPathComponent()] = nil
    }
    ...
    private func persistConfiguration() {
        ...
        guard let data = try? JSONEncoder.fx.encode(configuration) else { return }
    ...
    private func removePreviousCoverFiles(for record: CaptureRecord, in itemURL: URL, keeping fileNames: Set<String>) {
