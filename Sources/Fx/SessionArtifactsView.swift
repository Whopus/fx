...
final class SessionArtifactTreeModel: ObservableObject {
    ...
    private var directoryRequests: [URL: UUID] = [:]
    ...
    func configure(
        ...
        directoryRequests.removeAll(keepingCapacity: true)
    ...
    func loadDirectory(_ directoryURL: URL, force: Bool = false) async {
        ...
        let request = UUID()
        directoryRequests[directoryURL] = request
        loadingDirectories.insert(directoryURL)
        defer {
            if directoryRequests[directoryURL] == request {
                directoryRequests[directoryURL] = nil
                loadingDirectories.remove(directoryURL)
            }
        }
        ...
        do {
            ...
            let entries = try await Self.scanDirectory(
                directoryURL,
                excluding: excludedPaths,
                excludingManagedItemDirectories: excludesManagedDirectories
            )
            guard !Task.isCancelled, revision == configurationRevision,
                  directoryRequests[directoryURL] == request else { return }
            guard directoryURL == rootURL || childrenByDirectory.keys.contains(where: {
                directoryURL.path.hasPrefix($0.path + "/")
            }) else { return }
            if childrenByDirectory[directoryURL] != entries { childrenByDirectory[directoryURL] = entries }
        } catch {
            guard !Task.isCancelled, revision == configurationRevision,
                  directoryRequests[directoryURL] == request else { return }
    ...
    func refreshLoadedDirectories() async {
        ...
        let revision = configurationRevision
        let request = UUID()
        for directory in loadedDirectories { directoryRequests[directory] = request }
        defer {
            for directory in loadedDirectories where directoryRequests[directory] == request {
                directoryRequests[directory] = nil
                if loadingDirectories.contains(directory) { loadingDirectories.remove(directory) }
            }
        }
        var refreshed: [URL: [SessionArtifactEntry]] = [:]
        for directoryURL in loadedDirectories {
            guard !Task.isCancelled, revision == configurationRevision else { return }
            if let entries = try? await Self.scanDirectory(
                directoryURL,
                ...
                refreshed[directoryURL] = entries
            }
        }
        guard !Task.isCancelled, revision == configurationRevision else { return }
        var updated = childrenByDirectory
        for (directoryURL, entries) in refreshed {
            guard directoryRequests[directoryURL] == request else { continue }
            updated[directoryURL] = entries
        }
        if updated != childrenByDirectory { childrenByDirectory = updated }
    }
    ...
    ) async throws -> [SessionArtifactEntry] {
...
struct ContextSessionArtifactsView: View {
    ...
    @State private var requestedRefresh = 0
    ...
    var body: some View {
        ...
        .task(id: requestedRefresh) {
            guard requestedRefresh > 0 else { return }
    ...
    private func writeToPasteboard(_ url: URL, operation: String) {
        ...
        pasteboard.setString(
            operation,
            forType: NSPasteboard.PasteboardType("org.fx.file-operation")
        )
    }
