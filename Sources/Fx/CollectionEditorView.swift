...
struct CollectionEditorView: View {
    ...
    @State private var insertionMenuIndex: Int?
    ...
    @State private var compressionDraft: ContextCompressionDraft?
    @State private var compressionCancellation: ContextRunCancellation?
    ...
    var body: some View {
        ...
        .onDisappear {
            ...
            compressionCancellation?.cancel()
            compressionTask?.cancel()
            documentScrollCache.onActiveItemChange = nil
    ...
    private func sidebarButton(_ item: ContextNotebookItem, index: Int) -> some View {
        ...
        let hovered = item.id == hoveredItemID || replacementMenuID == item.id
        return HStack(spacing: 8) {
            ...
            HStack(spacing: 0) {
                GlassMenu(entries: replaceMenuEntries(itemID: item.id), accessibilityTitle: "替换 item",
                    isPresented: Binding(get: { replacementMenuID == item.id }, set: { replacementMenuID = $0 ? item.id : nil })) {
    ...
    private func insertionControl(at index: Int) -> some View {
        let visible = hoveredInsertionIndex == index || insertionMenuIndex == index
        return HStack(spacing: 6) {
            Rectangle()
                .fill(visible ? Color.black.opacity(0.28) : Color.clear)
                .frame(height: 1)

            GlassMenu(entries: addMenuEntries(insertionIndex: index), accessibilityTitle: "添加 item",
                isPresented: Binding(get: { insertionMenuIndex == index }, set: { insertionMenuIndex = $0 ? index : nil })) {
    ...
    private func addMenuEntries(insertionIndex: Int?) -> [GlassMenuEntry] {
    ...
    private var compressionWorkspace: some View {
        VStack(spacing: 0) {
            HStack(spacing: 12) {
                ...
                if compressionDraft?.isRunning == true {
                    ProgressView().controlSize(.small)
                    Text("AI 正在压缩 System 和 Context…")
                        .font(.system(size: 12))
                        .foregroundStyle(.secondary)
                } else {
                    Text("检查右侧结果后应用")
                ...
                Button("应用压缩") { applyCompression() }
                    .buttonStyle(.borderedProminent)
                    .controlSize(.small)
                    .disabled(compressionDraft?.isRunning != false)
            }
            ...
            ScrollView {
                LazyVStack(spacing: 0) {
                    if compressionDraft != nil {
                        ForEach(notebook.items.map(ContextCompressionSection.init)) { original in
                            compressionRow(original)
                        }
                    }
                }
            }
    ...
    private func compressionRow(_ original: ContextCompressionSection) -> some View {
        let isCompressible = original.kind == .system || original.kind == .context
        let proposal = compressionDraft?.proposed.first(where: { $0.id == original.id }) ?? original
        let changed = proposal != original
        return HStack(alignment: .top, spacing: 0) {
            VStack(alignment: .leading, spacing: 12) {
                compressionSectionHeader(original, changed: changed, isOriginal: true)
                Text(original.displayText.isEmpty ? " " : original.displayText)
            ...
            VStack(alignment: .leading, spacing: 10) {
                compressionSectionHeader(proposal, changed: changed, isOriginal: false)
                if !isCompressible {
                    Text(original.displayText.isEmpty ? " " : original.displayText)
                        .font(.system(size: 12.5, design: .monospaced))
                        .foregroundStyle(.primary.opacity(0.76))
                        .textSelection(.enabled)
                        .frame(maxWidth: .infinity, alignment: .topLeading)
                } else if compressionDraft?.isRunning == true {
                    HStack(spacing: 8) {
                    ...
                    HStack {
                        ...
                        Button("恢复") { restoreCompressionSection(original) }
                            .buttonStyle(.plain)
                            .font(.system(size: 11, weight: .medium))
                    }
                    .frame(height: 30)
                } else {
                    compressionSectionEditors(proposal)
                }
            }
            .padding(18)
            .frame(maxWidth: .infinity, alignment: .topLeading)
        ...
        .background {
            HStack(spacing: 0) {
                changed ? Color.red.opacity(0.045) : Color.clear
                changed ? Color.green.opacity(0.055) : Color.clear
            }
        }
        .overlay {
            Rectangle().fill(.black.opacity(0.09)).frame(width: 1)
        }
    ...
    private func compressionSectionHeader(
        _ section: ContextCompressionSection,
        changed: Bool,
        isOriginal: Bool
    ) -> some View {
        HStack(spacing: 7) {
            Image(systemName: section.kind.icon)
            Text(section.kind.menuLabel)
            Text(section.id.uuidString.prefix(8))
                .font(.system(size: 10, design: .monospaced))
                .foregroundStyle(.tertiary)
            Spacer()
            if !isOriginal && section.kind != .system && section.kind != .context {
                Text("原样保留")
            } else if changed {
                Text(isOriginal ? "原文" : section.isEmpty ? "删除" : "已压缩")
                    .foregroundStyle(isOriginal ? Color.red.opacity(0.72) : Color.green.opacity(0.8))
            }
        }
    ...
    @ViewBuilder
    private func compressionSectionEditors(_ section: ContextCompressionSection) -> some View {
        if !section.title.isEmpty {
            TextField("Title", text: compressionBinding(for: section.id, keyPath: \.title))
                .textFieldStyle(.plain)
                .font(.system(size: 15, weight: .medium))
        }
        if !section.detail.isEmpty {
            TextField("Detail", text: compressionBinding(for: section.id, keyPath: \.detail), axis: .vertical)
                .textFieldStyle(.plain)
                .font(.system(size: 12.5))
                .foregroundStyle(.secondary)
        }
        TextField("空白内容", text: compressionBinding(for: section.id, keyPath: \.body), axis: .vertical)
        .textFieldStyle(.plain)
        .font(.system(size: 12.5, design: .monospaced))
        .foregroundStyle(.primary.opacity(0.76))
        .lineLimit(1...)
    ...
    private func compressionBinding(
        for id: UUID,
        keyPath: WritableKeyPath<ContextCompressionSection, String>
    ) -> Binding<String> {
        Binding(
            get: {
                compressionDraft?.proposed.first(where: { $0.id == id })?[keyPath: keyPath] ?? ""
            },
            set: { value in
                guard let index = compressionDraft?.proposed.firstIndex(where: { $0.id == id }) else { return }
                compressionDraft?.proposed[index][keyPath: keyPath] = value
            }
        )
    }

    private func restoreCompressionSection(_ original: ContextCompressionSection) {
        guard let index = compressionDraft?.proposed.firstIndex(where: { $0.id == original.id }) else { return }
        compressionDraft?.proposed[index] = original
    }

    private func startCompression() {
        guard !isRunning, compressionDraft == nil, let workingDirectory = runtimeWorkingDirectoryURL else { return }
        NSApp.keyWindow?.makeFirstResponder(nil)
        dismissModelPicker()
        let originals = ContextCompression.sections(from: notebook)
        guard !originals.isEmpty else { return }
        compressionDraft = ContextCompressionDraft(originals: originals, proposed: originals, isRunning: true)
        let request: ContextNotebook
        do {
            request = try ContextCompression.requestNotebook(for: notebook)
        } catch {
        ...
        let payload = ContextRuntimePayload(
            notebook: request,
            collectionURL: workingDirectory,
            contexts: [:],
            mediaURLs: [:]
        )
        let cancellation = ContextRunCancellation()
        compressionCancellation = cancellation
        compressionTask = Task { @MainActor in
            defer {
                if compressionCancellation === cancellation {
                    compressionCancellation = nil
                    compressionTask = nil
                }
            }
            do {
                let result = try await ContextPiRunner.run(payload, cancellation: cancellation)
                guard !cancellation.isCancellationRequested else { return }
                let proposals = try ContextCompression.decode(result.final, matching: originals)
                guard compressionDraft?.originals == originals else { return }
                compressionDraft = ContextCompressionDraft(
                    originals: originals,
                    proposed: proposals,
                    isRunning: false
                )
    ...
    private func applyCompression() {
        guard let draft = compressionDraft, !draft.isRunning else { return }
        do {
            let updated = try ContextCompression.applying(draft.proposed, to: notebook)
            notebook = updated
            let session = try store.upsertSession(notebook: updated, sessionID: activeSessionID)
            try activateSessionWorkspace(session)
            shouldKeepSessionOnClose = true
            selectedItemID = updated.items.first?.id
            compressionDraft = nil
        } catch {
            errorMessage = error.localizedDescription
        }
    ...
    private var runtimeToolbar: some View {
        HStack(alignment: .center, spacing: 14) {
            ...
            GlassMenu(entries: modelMenuEntries, width: 238, accessibilityTitle: "选择模型",
                isPresented: $isModelPickerPresented, onOpen: { modelPickerLevelSpec = nil }) {
                HStack(spacing: 6) {
                    Circle()
            ...
            HStack(spacing: 0) {
                ...
                GlassMenu(entries: ContextPrimaryAction.allCases.map { action in
                    GlassMenuEntry(id: action.rawValue, title: action.rawValue,
                        icon: primaryAction == action && action != .run ? "checkmark" : action.icon) {
                            primaryAction = action
                            didSaveSession = false
                    }
                } + [
                    GlassMenuEntry(
                        id: "compress-context", title: "compact",
                        icon: "arrow.left.and.right.text.vertical",
                        enabled: !isRunning && compressionDraft == nil
                            && notebook.items.contains { $0.kind == .system || $0.kind == .context },
                        action: startCompression
                    )
                ], width: 160, accessibilityTitle: "选择操作") {
                    Image(systemName: "chevron.down")
                        .symbolRenderingMode(.monochrome)
    ...
    private var modelMenuEntries: [GlassMenuEntry] {
        if let spec = modelPickerLevelSpec,
           let model = modelOption(for: spec), !model.reasoningEfforts.isEmpty {
            return [
                GlassMenuEntry(id: "reasoning-back", title: model.name, icon: "chevron.left",
                    detail: "Reasoning", keepsOpen: true) { modelPickerLevelSpec = nil },
                .separator()
            ] + model.reasoningEfforts.map { effort in
                GlassMenuEntry(id: "effort-\(model.spec)-\(effort.rawValue)", title: effort.displayName,
        ...
        var entries = fxModelSettings.models.map { model in
            let selected = selectedModelSpec == model.spec
            return GlassMenuEntry(id: "model-\(model.spec)", title: model.name,
                detail: selected && !model.reasoningEfforts.isEmpty ? selectedReasoningEffort.displayName : nil,
                selected: selected, disclosure: !model.reasoningEfforts.isEmpty,
                keepsOpen: !model.reasoningEfforts.isEmpty) {
        ...
        if entries.isEmpty { entries.append(.heading("No configured models")) }
        entries.append(.separator())
        if !notebook.model.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            entries.append(GlassMenuEntry(id: "settings-default", title: "Use Settings Default",
                icon: "arrow.uturn.backward") {
                    notebook.model = ""
                    dismissModelPicker()
                })
        }
        entries.append(GlassMenuEntry(id: "reload-models", title: "Reload Configuration", icon: "arrow.clockwise") {
            ...
            LazyHStack(spacing: 8) {
                ForEach(item.wrappedValue.attachments ?? []) { attachment in
                    ZStack(alignment: .topTrailing) {
                        ContextQueryAttachmentImage(data: attachment.data)
                        .equatable()
...
/// Prompt edits do not change attachment bytes. Isolating this leaf prevents
/// recreating NSImage for every keystroke while keeping the same rendered view.
private struct ContextQueryAttachmentImage: View, Equatable {
    let data: Data
...
private struct ContextFastTextEditor: NSViewRepresentable {
    ...
    func updateNSView(_ scrollView: NSScrollView, context: Context) {
        ...
        let typographyChanged = previous.fontSize != fontSize || previous.lineSpacing != lineSpacing
            || previous.usesPrimaryTextColor != usesPrimaryTextColor
        ...
        if textView.textColor != contentColor { textView.textColor = contentColor }
        textView.onPasteImage = onPasteImage

        if typographyChanged {
            applyTypography(to: textView)
        }
        if typographyChanged || previous.minHeight != minHeight || previous.maximumLines != maximumLines {
...
private final class ContextShimmerLabelView: NSView {
    ...
    override func viewDidMoveToWindow() {
        ...
        if window == nil {
            gradient.removeAnimation(forKey: "fx-shimmer")
        } else {
            startAnimationIfNeeded()
        }
    }
    ...
    private func startAnimationIfNeeded() {
        guard window != nil,
              bounds.width > 0,
              gradient.animation(forKey: "fx-shimmer") == nil else { return }
        ...
        gradient.add(group, forKey: "fx-shimmer")
    }
}
...
private extension JSONValue {
    ...
    var displayText: String {
        ...
        guard let data = try? JSONEncoder.fx.encode(self),
