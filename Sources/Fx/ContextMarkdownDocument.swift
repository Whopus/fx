...
enum ContextMarkdownParser {
    ...
    private static func table(
        ...
        let meaningfulColumns = headers.indices.filter { column in
            !headers[column].trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
                || rows.contains { row in
                    row.indices.contains(column)
                        && !row[column].trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
                }
        }
        guard !meaningfulColumns.isEmpty else { return nil }

        let normalizedHeaders = meaningfulColumns.map { headers[$0] }
        let normalizedAlignments = meaningfulColumns.map { alignments[$0] }
        let normalizedRows = rows.map { row in
            meaningfulColumns.map { row.indices.contains($0) ? row[$0] : "" }
        }

        return (
            .table(headers: normalizedHeaders, alignments: normalizedAlignments, rows: normalizedRows),
            nextIndex
        )
    }
...
struct ContextMarkdownDocument: View, Equatable {
    ...
    /// Compact, block-aware rhythm scaled for Fx's 13pt editor body.
...
private struct ContextMarkdownTableView: View {
    let headers: [String]
    let alignments: [ContextMarkdownTableAlignment]
    let rows: [[String]]
    let baseFontSize: CGFloat
    let tone: ContextMarkdownTone
    ...
    private var columnWidths: [CGFloat] {
        headers.indices.map { column in
            ...
            let desired = values.map(estimatedWidth).max() ?? 0
            return min(max(desired + 22, 112), headers.count <= 3 ? 420 : 320)
        }
    }
    ...
    private func cell(
        ...
        Text(ContextMarkdownRenderCache.shared.inline(for: source))
    ...
    private func estimatedWidth(_ source: String) -> CGFloat {
        let widestLine = source.split(separator: "\n", omittingEmptySubsequences: false)
            .map { line in
                line.reduce(CGFloat.zero) { width, character in
                    if "*_~`".contains(character) { return width }
                    let isASCII = character.unicodeScalars.allSatisfy(\.isASCII)
                    return width + (isASCII ? 0.58 : 1)
                }
            }
            .max() ?? 0
        return widestLine * baseFontSize
    }

    private func cellAlignment(_ alignment: ContextMarkdownTableAlignment) -> Alignment {
        switch alignment {
    ...
    private func textAlignment(_ alignment: ContextMarkdownTableAlignment) -> TextAlignment {
        switch alignment {
