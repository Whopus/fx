...
final class AppServices: ObservableObject {
    ...
    let islandActivity: FxIslandActivityController
...
final class CaptureCoordinator: NSObject, ObservableObject {
    ...
    private let islandActivity: FxIslandActivityController
    ...
    init(store: CaptureStore, islandActivity: FxIslandActivityController) {
        self.store = store
        self.islandActivity = islandActivity
        super.init()
    }
    ...
    private func startCaptureObservation() {
        ...
        // Scheduled on the main run loop. Weak ownership avoids the repeating
        // timer keeping its coordinator alive when services are released.
    ...
    func toggleCaptureMode() {
        ...
        if isCaptureModeEnabled {
            ...
            if AXIsProcessTrusted() {
                ...
                showStatus("采集模式已开启 · 辅助功能权限未生效时请在系统设置中重新勾选 Fx")
            }
        } else {
            stopCaptureObservation()
    ...
    private func present(candidate: CaptureCandidate, fingerprint: String) {
        ...
        islandActivity.set(FxIslandActivity(task: task, status: .needsAttention))
    ...
    private func performBrowserSnapshot() async {
        let task = FxTaskKind.browserSnapshot
        islandActivity.set(FxIslandActivity(task: task, status: .processing))
    ...
    private var restingIslandActivity: FxIslandActivity {
        isCaptureModeEnabled ? .captureReady : .idle
    }

    private func taskKind(for candidate: CaptureCandidate) -> FxTaskKind {
        switch candidate {
    ...
    private func showTaskFailure(_ task: FxTaskKind) {
        islandActivity.showTransient(
            FxIslandActivity(task: task, status: .failed),
            revertingTo: restingIslandActivity
        )
