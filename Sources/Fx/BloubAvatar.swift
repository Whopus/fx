    ...
    var body: some View {
        TimelineView(.animation(minimumInterval: 1 / 60, paused: isPaused)) { timeline in
            ...
            Canvas(opaque: false, rendersAsynchronously: true) { context, size in
                ...
                let oldFrame = progress < 1 ? BloubSampler.sample(
                ...
                BloubRenderer.draw(
                    oldFrame.map { BloubFrame.interpolate(from: $0, to: newFrame, progress: progress) } ?? newFrame,
                    in: size,
                    palette: palette,
                    context: &context
                )
...
struct BloubPointerTarget: Equatable {
    ...
    static func resolve(
        ...
        // customizer, Fx has no side panel, so there is no fixed turn.
...
private enum BloubSampler {
    ...
    private static let circularProfile = Array(repeating: CGFloat(1), count: sampleCount)
    ...
    static func sample(
        ...
        if presentation.expression == .neutral,
           ...
           let blend = BloubIdleSequence.blend(
            ...
            if blend.progress == 1 {
                return sampleBase(blend.to, at: time, gaze: gaze)
            }
    ...
    private static func sampleBase(
        ...
        let profile = deformation == 0 ? circularProfile : (0..<sampleCount).map { index in
