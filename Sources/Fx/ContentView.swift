...
struct ContentView: View {
    ...
    private var header: some View {
        HStack(spacing: 0) {
            ...
            ItemTypeMenu(selection: $gallerySpace, records: captureStore.records)
            .padding(.leading, 8)
            .offset(x: 12)
        }
    ...
    private var gallery: some View {
        let snapshot = captureStore.gallerySnapshot(in: gallerySpace)
    ...
    var body: some View {
        HStack(spacing: 8) {
            Button {
                ...
                .glassEffect(
                    .regular
                        .tint(captureCoordinator.isCaptureModeEnabled ? .black.opacity(0.86) : nil)
                        .interactive(),
                    in: Capsule()
                )
            }
        ...
        Group {
            if let message = captureCoordinator.statusMessage {
                Text(message)
                    ...
                    .frame(height: 34)
                    .glassEffect(.regular.tint(.black.opacity(0.86)), in: Capsule())
                    .padding(.top, 84)
