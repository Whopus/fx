...
extension GalleryItem {
    init(
        ...
        let pixelRatio = min(2.2, max(0.62, width / max(1, height)))
        // A text/link item can carry a cover image. The cover's own proportions
        // drive the card, not the text placeholder ratio; otherwise the masonry
        // reserves a short landscape frame while the card renders a tall cover,
        // so the gallery overflows without the scroll view knowing it should
        // scroll.
        let coverRatio = coverPreviewURL.flatMap(Self.pixelAspectRatio(of:))
            .map { min(2.4, max(0.5, $0)) }
        let aspectRatio = coverRatio
            ?? (capture.kind == .text || capture.kind == .link ? 1.15 : pixelRatio)
    ...
    /// Reads a local cover's pixel aspect ratio from its metadata without
    /// decoding the image, honoring the EXIF orientation so portrait covers
    /// stay portrait.
    static func pixelAspectRatio(of url: URL) -> CGFloat? {
        guard url.isFileURL,
        ...
        return CGFloat(isRotated ? height / width : width / height)
    }
