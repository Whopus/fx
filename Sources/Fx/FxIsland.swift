...
private final class PresentedFrameCompletion: @unchecked Sendable {
    ...
    /// `MTLDrawable.addPresentedHandler` invokes its callback on a private Core
    /// Animation queue on macOS 26. Keep the callback itself nonisolated, then
    /// explicitly hop to the main actor before touching AppKit state.
    nonisolated func install(on drawable: MTLDrawable) {
        drawable.addPresentedHandler { [self] _ in
            Task { @MainActor in
                body()
            }
        }
...
private final class DisplayLinkAnimationDriver: NSObject {
    ...
    func start(for view: NSView, update: @escaping (CFTimeInterval) -> Bool) {
        ...
        let maximumFPS = Float(max(view.window?.screen?.maximumFramesPerSecond ?? 60, 60))
...
@MainActor
final class FxIslandWindowController {
    ...
    func observe(_ activityController: FxIslandActivityController) {
        guard self.activityController !== activityController else { return }
        let wasVisible = panel?.isVisible == true
        self.activityController.setIslandVisible(false)
        self.activityController = activityController
        if let panel {
            islandContainer(in: panel)?.setActivityController(activityController)
        }
        activityController.setIslandVisible(wasVisible)
    }
    ...
    func collapse(
        ...
        guard shouldAnimate, let screen, let snapshot = snapshot(of: window) else {
            window.orderOut(nil)
            show(on: screen)
            completion()
            return
    ...
    private func islandContainer(in panel: NSPanel) -> FxIslandContainerView? {
        panel.contentView as? FxIslandContainerView
    }
    ...
    private func makePanel() -> FxIslandPanel {
        let panel = FxIslandPanel(
            contentRect: NSRect(origin: .zero, size: islandSize),
            styleMask: [.borderless, .nonactivatingPanel],
            backing: .buffered,
            defer: false
        )
        panel.contentView = FxIslandContainerView(
...
private final class GenieTransitionView: MTKView, MTKViewDelegate {
    ...
    init(
        ...
        isPaused = true
        preferredFramesPerSecond = max(NSScreen.main?.maximumFramesPerSecond ?? 60, 60)
    ...
    override func viewDidMoveToWindow() {
        ...
        // Drive the transition at the attached display's native cadence. This
        // automatically uses 120 Hz on ProMotion displays without oversampling
        // 60 Hz panels or introducing a second, unsynchronised timer.
        preferredFramesPerSecond = max(window?.screen?.maximumFramesPerSecond ?? 60, 60)
    ...
    func draw(in view: MTKView) {
        ...
        if didFinish {
            ...
            if let completion {
                ...
                presentedCompletion.install(on: drawable)
            }
        }
    ...
    private static func makeIslandMaskTexture(
        ...
        context.addPath(fxIslandPath(in: CGRect(origin: .zero, size: size)))
        context.fillPath()
...
private func fxIslandPath(in rect: CGRect) -> CGPath {
...
private final class FxIslandPanel: NSPanel {
    override init(
        contentRect: NSRect,
...
private final class FxIslandContainerView: NSView {
    private let backgroundView = NSHostingView(rootView: FxIslandBackgroundView())
    private let hostingView: NSHostingView<FxIslandView>

    init(frame frameRect: NSRect, activityController: FxIslandActivityController) {
        hostingView = NSHostingView(
            rootView: FxIslandView(activityController: activityController)
        )
    ...
    func setActivityController(_ activityController: FxIslandActivityController) {
        hostingView.rootView = FxIslandView(activityController: activityController)
    }
}

private struct FxIslandBackgroundView: View {
    var body: some View {
        Color.black
            .clipShape(FxIslandShape())
...
private struct FxIslandView: View {
    @ObservedObject var activityController: FxIslandActivityController
...
private struct FxIslandShape: Shape {
