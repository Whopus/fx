...
struct CaptureDetailView: View {
    ...
    @State private var textRefreshRevision = 0
    @State private var copiesRefreshedContext = false
    @State private var didCopyContext = false
    @State private var copyFeedbackGeneration = 0
    @State private var isCoverHovered = false
    @State private var isCoverMenuPresented = false
    ...
    var body: some View {
        ...
        .task(id: TextRequest(record: record, revision: textRefreshRevision)) {
            guard let record else { return }
            if await loadTextSnapshot(for: record), copiesRefreshedContext {
                copiesRefreshedContext = false
                copyCurrentContext()
            }
        }
        .onReceive(NotificationCenter.default.publisher(for: NSApplication.didBecomeActiveNotification)) { _ in
            guard selectedTab == .defaultTab else { return }
            textRefreshRevision &+= 1
        }
        ...
        VStack(spacing: 0) {
            HStack(spacing: 8) {
                ScrollView(.horizontal) {
                    HStack(spacing: 4) {
                        ...
                        GlassMenu(entries: [
                            GlassMenuEntry(id: "text", title: "文本", icon: "doc.text") { beginCreatingTab(kind: .plainText) },
                            GlassMenuEntry(id: "markdown", title: "Markdown", icon: "doc.richtext") { beginCreatingTab(kind: .markdown) },
                            .separator(),
                            GlassMenuEntry(id: "image", title: "图片…", icon: "photo") { importFileTab(kind: .image) },
                            GlassMenuEntry(id: "video", title: "视频…", icon: "play.rectangle") { importFileTab(kind: .video) },
                            GlassMenuEntry(id: "file", title: "其他文件…", icon: "doc.badge.plus") { importFileTab() }
                        ], accessibilityTitle: "新建内容 Tab") {
            ...
            if isCreatingTab {
                HStack(spacing: 10) {
                    ...
                    GlassMenu(entries: [CaptureDetailTab.Kind.markdown, .plainText].map { kind in
                        GlassMenuEntry(id: kind.rawValue, title: kind.displayName, selected: newTabKind == kind) {
                            newTabKind = kind
                        }
                    }, width: 180, accessibilityTitle: "类型") {
                        HStack {
                            Text(newTabKind.displayName)
                            Spacer(minLength: 4)
                        ...
                        .background(.black.opacity(0.055), in: RoundedRectangle(cornerRadius: 5))
                    }
                    .frame(width: 120)
    ...
    private func detailHero(_ record: CaptureRecord) -> some View {
        HStack(alignment: .top, spacing: 0) {
            ...
            .overlay(alignment: .topLeading) {
                if isCoverHovered || isCoverMenuPresented {
    ...
    private var itemTypeMenu: some View {
        GlassMenu(entries: CaptureSpace.allCases.map { space in
            GlassMenuEntry(id: space.rawValue, title: space.displayName, icon: space.icon, selected: itemType == space) {
                itemType = space
            }
        }, accessibilityTitle: "修改 Item 类型") {
            HStack(spacing: 6) {
    ...
    private func coverHoverControl(_ record: CaptureRecord) -> some View {
        let entries: [GlassMenuEntry] = [
            GlassMenuEntry(id: "image", title: "图片封面…", icon: "photo") { chooseImageCover() },
            GlassMenuEntry(id: "video", title: "视频封面…", icon: "video") { chooseVideoCover() }
        ] + (record.coverFileName == nil ? [] : [
            .separator(),
            GlassMenuEntry(id: "remove", title: "移除封面", icon: "xmark", destructive: true) { removeCover() }
        ])
        return GlassMenu(entries: entries, accessibilityTitle: "设置图片或视频封面", isPresented: $isCoverMenuPresented) {
    ...
    private func loadTextSnapshot(for record: CaptureRecord) async -> Bool {
        let revision = textRefreshRevision
        let source = store.textSource(for: record)
        guard let snapshot = try? await source.loadDetail(),
              !Task.isCancelled, self.record == record,
              textRefreshRevision == revision,
              store.fileURL(for: record) == source.originalURL else { return false }
    ...
    private func saveAndCopyContext() {
        guard persistDefaultFields() else { return }
        copiesRefreshedContext = true
        textRefreshRevision &+= 1
    }
