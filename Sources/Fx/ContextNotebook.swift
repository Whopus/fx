...
struct ContextNotebook: Codable, Equatable, Sendable {
    static let fileName = ".fx-context.json"
...
struct FxModelSettings: Equatable, Sendable {
    ...
    static let runtimeDefaults = FxModelSettings(
        ...
        models: [
            ...
            FxModelOption(spec: "cheaperinference/deepseek-v4.1-flash", name: "DeepSeek V4.1 Flash (Cheaper Inference)"),
...
enum ContextNotebookRepository {
    ...
    static func load(from collectionURL: URL, title: String) throws -> ContextNotebook {
        ...
        var notebook = try JSONDecoder.fx.decode(ContextNotebook.self, from: Data(contentsOf: fileURL))
        ...
        if notebook.version < 4 {
            // Rename only Fx's untouched built-in prompt. User-authored System
    ...
    static func save(_ notebook: ContextNotebook, to collectionURL: URL) throws {
        try JSONEncoder.fx.encode(notebook).write(
...
enum ContextNotebookError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .runtimeNotFound:
            "找不到 Fx 内置 Agent Runtime。请重新构建 Fx.app。"
        case .nodeNotFound:
            "找不到 Node.js。Fx Agent Runtime 需要 Node.js 22.19 或更高版本。"
        case .malformedOutput:
...
struct ContextRuntimePreparation: Sendable {
    ...
    // Runs on the generic executor under the package's Swift 6 settings.
    func load() async throws -> ContextRuntimePayload {
        try Task.checkCancellation()
        var contexts: [UUID: String] = [:]
        for source in textSources {
            try Task.checkCancellation()
            contexts[source.record.id] = source.context()
        }
        let messages = continuationURL.flatMap { url -> [JSONValue]? in
            guard let data = try? Data(contentsOf: url) else { return nil }
            return try? JSONDecoder.fx.decode([JSONValue].self, from: data)
        }
        try Task.checkCancellation()
...
enum ContextPiRunner {
    static func run(
        ...
        let cancellation = cancellation ?? ContextRunCancellation()
    ...
    private static func execute(
        _ payload: ContextRuntimePayload,
        cancellation: ContextRunCancellation,
        onEvent: @escaping @MainActor @Sendable (ContextRunEvent) -> Void
    ) async throws -> ContextRunResult {
        ...
        let notebookURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("fx-\(runID).runtime.json")
        let logURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("fx-\(runID).log")
        let eventLogURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("fx-\(runID).events.ndjson")
        ...
        // The reader is a child of this operation and is joined on every exit.
        // Decoding remains on the generic executor; only batches enter MainActor.
        async let eventDelivery: Void = readEvents(from: outputPipe.fileHandleForReading, onEvent: onEvent)
        ...
        do {
            status = try await withCheckedThrowingContinuation { continuation in
                ...
                do {
                    guard !cancellation.isCancellationRequested else {
                        process.terminationHandler = nil
                        continuation.resume(throwing: ContextNotebookError.cancelled)
                        return
                    }
                    try process.run()
        ...
        do {
            for try await line in handle.bytes.lines {
                guard let data = line.data(using: .utf8),
                      let event = try? decoder.decode(ContextRunEvent.self, from: data) else { continue }
                for ready in batcher.consume(event) { await onEvent(ready) }
            }
        } catch {
            // The final notebook is authoritative if stdout closes mid-line.
        }
        for ready in batcher.flush() { await onEvent(ready) }
    }
    ...
    private static func jsonObject<T: Encodable>(_ value: T) throws -> Any {
        try JSONSerialization.jsonObject(with: JSONEncoder.fx.encode(value))
    }
    ...
    private static func findRuntimeCLI() throws -> URL {
        ...
        var candidates: [URL] = []
        if let configured = ProcessInfo.processInfo.environment["FX_RUNTIME_PATH"] {
            let url = URL(fileURLWithPath: configured)
            candidates.append(url.hasDirectoryPath ? url.appendingPathComponent("dist/cli.js") : url)
        }
        if let resources = Bundle.main.resourceURL {
            candidates.append(resources.appendingPathComponent("FxRuntime/dist/cli.js"))
        }
        candidates.append(
