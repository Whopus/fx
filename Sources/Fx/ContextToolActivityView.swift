    ...
    static func make(
        ...
        if isResult {
            let fallbackResult = cleanResult(
                family == .bash ? firstLines(of: fallbackText, limit: 1) : fallbackText
            )
            ...
            return ContextToolPresentation(
                ...
                result: fallbackResult.isEmpty
                    ? cleanResult(family == .bash ? firstLines(of: payloadResult, limit: 1) : payloadResult)
                    : fallbackResult,
                diff: .empty
            )
        }
        ...
        switch family {
        case .bash:
            summary = string(in: object, keys: ["intent"]) ?? "Run shell command"
            metadata = number(in: object, keys: ["timeout"]).map { "timeout \(compactNumber($0))s" }
            command = string(in: object, keys: ["command", "cmd", "script"])
...
struct ContextToolActivityView: View {
    ...
    @State private var isBashCommandExpanded = false
    ...
    private var canExpandResult: Bool {
        guard presentation.family != .read, presentation.family != .bash else { return false }
    ...
    private var genericBody: some View {
        VStack(alignment: .leading, spacing: 7) {
            ...
            if presentation.family == .bash, let summary = presentation.summary, !summary.isEmpty {
                Button {
                    withAnimation(.spring(duration: 0.36, bounce: 0.08)) {
                        isBashCommandExpanded.toggle()
                    }
                } label: {
                    HStack(spacing: 6) {
                        Image(systemName: "chevron.right")
                            .font(.system(size: 8.5, weight: .semibold))
                            .rotationEffect(.degrees(isBashCommandExpanded ? 90 : 0))
                            .frame(width: 9)
                        Text(summary)
                    }
                ...
                .buttonStyle(.plain)
                .help(isBashCommandExpanded ? "Hide command" : "Show command")
                .padding(.leading, 25)

                if isBashCommandExpanded, let command = presentation.command, !command.isEmpty {
                    Text(command)
                        ...
                        .padding(.leading, 25)
                        .transition(.contextVerticalReveal)
                }
            } else if !showsSummaryInline, let summary = presentation.summary, !summary.isEmpty {
                Text(summary)
            ...
            if resultPresentation != nil {
                ...
                HStack(alignment: .top, spacing: 8) {
                    ...
                    Text(visibleResult)
                        ...
                        .lineSpacing(3)
                        .lineLimit(presentation.family == .bash ? 1 : (isResultExpanded ? nil : 12))
                        .truncationMode(.tail)
                        .textSelection(.enabled)
                        .frame(maxWidth: .infinity, alignment: .leading)
                }
        ...
        .frame(maxWidth: .infinity, alignment: .leading)
        .animation(.spring(duration: 0.36, bounce: 0.08), value: isBashCommandExpanded)
    }
}

private struct ContextVerticalRevealModifier: @preconcurrency AnimatableModifier {
...
private extension AnyTransition {
    static var contextVerticalReveal: AnyTransition {
        .modifier(
            active: ContextVerticalRevealModifier(progress: 0),
            identity: ContextVerticalRevealModifier(progress: 1)
        )
    }
}
