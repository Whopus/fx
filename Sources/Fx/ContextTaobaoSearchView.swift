...
private func taobaoURL(_ value: JSONValue?) -> URL? {
    guard let rawValue = taobaoString(value),
          var components = URLComponents(string: rawValue) else { return nil }

    // Keep existing conversations renderable too: older tool results can
    // contain g.search[0-9]*.alicdn.com, whose TLS certificate is invalid for
    // that multi-label host. Alibaba serves the same path on img.alicdn.com.
    if let host = components.host?.lowercased(),
       host.hasPrefix("g.search"),
       host.hasSuffix(".alicdn.com") {
        let suffixStart = host.index(host.endIndex, offsetBy: -".alicdn.com".count)
        let variantStart = host.index(host.startIndex, offsetBy: "g.search".count)
        let variant = host[variantStart..<suffixStart]
        if variant.allSatisfy(\.isNumber) {
            components.host = "img.alicdn.com"
        }
    }
