...
You can also open `Package.swift` directly in Xcode and run the `Fx` scheme.

Requires macOS 26.2 or newer.

## Model menu

The model menu beside **Run** reads `~/.fx/settings.json` for compatibility with
existing local configuration. Model identifiers are passed to Fx's bundled
agent runtime as `provider/model`. For example:

```json
{
  "defaultModel": "wuai/gemini-3.8-flash",
  "models": [
    {
      "spec": "wuai/gemini-3.8-flash",
...
**default model**. Provider credentials are resolved inside Fx's runtime from
environment variables, macOS Keychain, `~/.fx/settings.json`, and
`~/.pi/agent/models.json`; secrets are never written into a Session.

## Agent runtime and Sessions

Fx owns the runtime under `Runtime/` and packages its compiled JavaScript,
...
zsh scripts/check-engineering.sh
swift test  # Requires a developer toolchain with XCTest, normally full Xcode.
```

The offline Swift harness uses temporary libraries and a local Node fixture; it
does not launch the GUI, read provider credentials, or contact model services.
It complements rather than replaces the full XCTest suite. See
[ENGINEERING_REVIEW.md](ENGINEERING_REVIEW.md) for the optimization rationale,
validation results, remaining findings, and on-device profiling checklist.

All in-app dropdowns share `GlassMenu`: clear glass, a soft shadow, a stronger hover
highlight, and a fade rather than a materialize transition. The materialize
transition scaled the scroll view that wraps the rows, which made long menus'
text jitter as they opened; the glass surface itself is unchanged. It does not
stretch the trigger into the menu. A transient hosting view above the
window content keeps card and scroll-view menus unclipped without creating another
window. The triggering control owns the presentation and its event-monitor lifetime.
Escape, outside clicks, keyboard selection, disabled/destructive rows, and model
reasoning levels use the same implementation. Right-click menus and system file
dialogs remain native. The offline checks cover navigation, classification/counts,
and placement, including narrow windows and long lists.

Interactive test windows and their launcher have been removed. The remaining
engineering checks above run without opening test interfaces.
