...
const keychainService = "com.fx.model.sub2api";
const keychainAccount = "fx";
...
async function ensureOpenAIGroup(token) {
  ...
  let group = listItems(listed).find((item) => item.platform === "openai" && item.name === "fx-openai");
  if (!group) {
    group = await request("/api/v1/admin/groups", {
      ...
      body: {
        name: "fx-openai",
        description: "Local Fx Codex gateway",
        platform: "openai",
        rate_multiplier: 1,
  ...
  if (!Number.isInteger(group?.id)) throw new Error("Unable to resolve the Fx OpenAI group.");
  return group.id;
}

async function ensureHostProxy(token) {
  const listed = await request("/api/v1/admin/proxies?page=1&page_size=100", { token });
  let proxy = listItems(listed).find((item) => item.name === "Fx Host Proxy");
  if (!proxy) {
  ...
  if (Number(administrator.balance ?? 0) < 1_000) {
    await request(`/api/v1/admin/users/${administrator.id}/balance`, {
      ...
      body: {
        balance: 10_000,
        operation: "set",
        notes: "Local-only Fx gateway accounting allowance",
      },
    });
  }
}
...
async function ensureAPIKey(token, groupID) {
  ...
  let key = listItems(listed).find((item) => item.name === "Fx Local Runtime" && item.group_id === groupID)?.key;
  if (!key) {
    const created = await request("/api/v1/keys", {
      token,
      method: "POST",
      body: { name: "Fx Local Runtime", group_id: groupID },
    });
    key = created?.key;
  }
